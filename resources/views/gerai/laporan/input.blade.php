{{-- resources/views/gerai/laporan/input.blade.php --}}
...
<div class="page">
    ...
    <main class="content">
        ...
        <div class="filter-card">
            ...
            <form class="filter-row" method="GET" action="{{ route('gerai.laporan.create') }}">
                <div class="f">
                    <div class="f-lbl">Tanggal Mulai (Senin)</div>
                    <input class="f-in" type="date" name="start" value="{{ $monday->toDateString() }}">
                </div>
        ...
        <form id="mainForm" method="POST" action="{{ route('gerai.laporan.store') }}">
            @csrf
            <input type="hidden" name="start" value="{{ $monday->toDateString() }}">
                        ...
                        <tbody>
                            ...
                            @foreach($dates as $d)
                                ...
                                <tr class="day-head">
                                    <td colspan="6">
                                        <b>{{ $no }}.</b>
                                        {{ $d->translatedFormat('l') }} — {{ $d->translatedFormat('d M Y') }}
                                        <span class="right">
                                            <button type="button" class="addRowBtn" data-date="{{ $key }}">＋ Tambah Jenis Layanan</button>
                                        </span>
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    <td colspan="6" style="padding:0;">
                                        <table class="subtbl" id="tbl-{{ $key }}">
                                            <tbody>
                                                @foreach($details as $i => $det)
                                                    <tr class="subrow">
                                                        <td style="width:60px; text-align:center; font-weight:900;">{{ $no }}</td>
                                                        <td style="width:120px;"></td>
                                                        <td style="width:160px;"></td>

                                                        <td>
                                                            <input class="in"
                                                                   name="items[{{ $key }}][{{ $i }}][jenis]"
                                                                   value="{{ $det->jenis_layanan ?? '' }}"
                                                        ...
                                                        <td class="qty">
                                                            ...
                                                            <input class="in num"
                                                                   name="items[{{ $key }}][{{ $i }}][jumlah]"
                                                                   value="{{ (int)($det->jumlah_pengguna ?? 0) }}"
                                                                   inputmode="numeric">
                                                        ...
                                                        <td>
                                                            <input class="in"
                                                                   name="items[{{ $key }}][{{ $i }}][ket]"
                                                                   value="{{ $det->keterangan ?? '' }}"
...
.content{ flex:1; padding: 22px; background: url('{{ asset('assets/bg-mpp.png') }}') center/cover fixed no-repeat; }
