...
<div class="admin-sidebar">
    ...
    <a class="admin-link {{ request()->is('admin/dashboard') ? 'active' : '' }}"
       href="{{ route('admin.dashboard') }}">
        <span class="icon">📈</span>
        Dashboard
    </a>

    {{-- ✅ Route belum ada? jangan panggil route() supaya tidak error --}}
    <a class="admin-link {{ request()->is('admin/laporan*') ? 'active' : '' }}"
       href="{{ Route::has('admin.laporan.index') ? route('admin.laporan.index') : '#' }}">
        <span class="icon">📄</span>
        Mengelola Laporan
    </a>

    <a class="admin-link {{ request()->is('admin/gerai*') ? 'active' : '' }}"
       href="{{ Route::has('admin.gerai.index') ? route('admin.gerai.index') : '#' }}">
        <span class="icon">🏢</span>
        Mengelola Gerai
    </a>

    <a class="admin-link {{ request()->is('admin/ranking*') ? 'active' : '' }}"
       href="{{ Route::has('admin.ranking.index') ? route('admin.ranking.index') : '#' }}">
        <span class="icon">🏆</span>
        Ranking Layanan
    </a>

    {{-- ✅ PESAN: kalau active → pakai warna header --}}
    <a class="admin-link {{ request()->is('admin/pesan*') ? 'active active-header' : '' }}"
